...
| `GET /` → `vendor_performance` block | Per-assignee speed & quality: avg repair hours, proof rejection rate (submitted proofs only — a human rejection or a non-overridden AI `irrelevant` verdict), and legacy resolved-on-arrival counts. Reads the `fixverify` schema directly (the sanctioned read-only cross-schema access in the shared PostgreSQL DB). |
...
| `POST /work-orders/{id}/proofs` | Multipart: `file`, `note?`. Requires the work order **started** (`in_progress`/`awaiting_proof`) — 409 otherwise. Runs the vision relevance check, stores the proof as a **draft** (`submitted=false`) and returns it with `{ai_verdict, ai_reason, ai_confidence}`. Nothing is finalised and no event fires — the uploader then submits or cancels. |
| `POST /proofs/{id}/submit` | `{override?: bool}`. Confirms a draft into human verification: work order → `pending_human_verification`, issue → `pending_verification`, emits `proof.uploaded`. A proof the AI is confident is `irrelevant` needs `override: true` (else HTTP 422 `{requires_override, ai_verdict, ai_reason}`); it is then flagged `ai_overridden`. |
| `DELETE /proofs/{id}` | Cancel/discard a draft — removes the file and row. 409 if the proof was already submitted. |
| `GET /proofs/{id}/file` | Serve the uploaded file. |
| `POST /proofs/{id}/human-verify` | Admin: `{approved: bool, notes?}`. The final say — offered on any submitted proof awaiting a verdict, including an `ai_overridden` one. Approved → issue `verified`, emits `issue.verified`; rejected → work order back to `awaiting_proof`, issue `in_progress`, emits `proof.rejected`. |
