...
| 2. Triage | System + Admin | AI + heuristics suggest severity and urgency class. Cross-referencing across all issues flags systemic faults (repeat issues on same floor, same equipment, same issue profile) — a flagged fault is drafted as a separate issue for an admin to review, edit and raise under their own name, detects duplicates (same defect reported by different users → escalate severity/urgency), and computes MTBF / MTTR metrics. Admin confirms or overrides. |
...
| `[*] → reported` (systemic escalation) | **Triage → Admin → Reporting** | Triage drafts an escalation issue for a systemic cluster; the admin accepts or edits it and sends, creating a **new issue they own as reporter** (`origin_cluster_id` set). It then follows the ordinary lifecycle. The triggering ticket is unaffected. See [05](05-triage-analytics.md). |
