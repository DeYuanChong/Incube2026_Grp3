...
| proof discarded (stays `in_progress`) | **Fix & Verify** | Vision model verdict `irrelevant` is shown to the uploader with the reason; if they discard rather than submit, the staged proof is deleted. No status change, no event. |
